...
use Evrinoma\VideoContentBundle\DependencyInjection\Compiler\Constraint\Complex\VideoContentPass;
use Evrinoma\VideoContentBundle\DependencyInjection\Compiler\Constraint\Property\VideoContentPass as PropertyVideoContentPass;
...
class EvrinomaVideoContentExtension extends Extension
{
    use HelperTrait;

    public const ENTITY = 'Evrinoma\VideoContentBundle\Entity';
    public const MODEL = 'Evrinoma\VideoContentBundle\Model';
    public const ENTITY_FACTORY_VIDEO_CONTENT = VideoContentFactory::class;
    public const ENTITY_BASE_VIDEO_CONTENT = BaseVideoContent::class;
    public const DTO_BASE_VIDEO_CONTENT = VideoContentApiDto::class;
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (self::ENTITY_FACTORY_VIDEO_CONTENT !== $config['factory']) {
    ...
    private function wireConstraintTag(ContainerBuilder $container): void
    {
        foreach ($container->getDefinitions() as $key => $definition) {
            switch (true) {
                case str_contains($key, PropertyVideoContentPass::VIDEO_CONTENT_CONSTRAINT)   :
                    $definition->addTag(PropertyVideoContentPass::VIDEO_CONTENT_CONSTRAINT);
                    break;
                case false !== strpos($key, VideoContentPass::VIDEO_CONTENT_CONSTRAINT):
                    $definition->addTag(VideoContentPass::VIDEO_CONTENT_CONSTRAINT);
                    break;
                default:
            }
        }
    ...
    private function wireRepository(ContainerBuilder $container, Reference $registry, string $madiator, string $class, string $driver): void
    {
        ...
        $container->addAliases([VideoContentCommandRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
        $container->addAliases([VideoContentQueryRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
    }
    ...
    {
        return EvrinomaVideoContentBundle::BUNDLE;
    }
}
